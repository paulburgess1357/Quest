...
namespace QuestGLCore::UniformBufferObjects {
	...
	public:
		...
		void send_data_to_buffer(const glm::vec2& data_to_send) const;
		void send_data_to_buffer(const glm::vec3& data_to_send) const;
		void send_data_to_buffer(const glm::vec4& data_to_send) const;
		void send_data_to_buffer(const glm::mat2& data_to_send) const;
		void send_data_to_buffer(const glm::mat3& data_to_send) const;
		void send_data_to_buffer(const glm::mat4& data_to_send) const;

	private:
		template<typename T>
		void send_data_to_buffer(const T& data_to_send) const {
			m_handle.bind();
