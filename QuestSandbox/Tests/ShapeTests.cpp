...
namespace QuestSandbox::Tests {
	...
	void ShapeTests::g_buffer_lighting_test_pointlight_mesh_volume() {
		...
		for (unsigned int i = 0; i < BOX_QTY; i++) {
			constexpr bool textured_cube_deferred = true;
			...
			load_dual_textured_cube_into_world({ xPos , yPos, zPos }, m_base_shader_path + "/GBuffer/GBufferShapeVertexGeometryPass.glsl", m_base_shader_path + "/GBuffer/GBufferShapeFragmentGeometryPass.glsl", textured_cube_deferred);
		}
		...
		for (unsigned int i = 0; i < LIGHT_QTY; i++) {
			...
			constexpr bool pointlight_mesh_deferred = true;
			...
			registry_api.load_model_into_world("pointlight_" + std::to_string(i), pointlight_model_ptr, { xPos , yPos, zPos }, pointlight_mesh_deferred); // deferred rendering
	...
	// ======================== Shape ========================
	void ShapeTests::load_dual_textured_cube_into_world(const glm::vec3& world_position, const std::string& vertex_shader, const std::string& fragment_shader, const bool deferred) {
		...
		const QuestEngine::API::RegistryAPI registry_api = m_engine_api.get_registry_api();
		registry_api.load_model_into_world(model_entity_id, model_pointer, world_position, deferred);
	}
