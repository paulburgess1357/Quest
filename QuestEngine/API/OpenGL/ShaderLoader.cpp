...
namespace QuestEngine::API::OpenGL {
	...
	void ShaderLoader::load_shader_program(const std::string& shader_program_name, std::string vertex, std::string fragment, const bool from_file) const {
		if(from_file) {
			vertex = QuestUtility::String::FileToString::load(vertex);
			fragment = QuestUtility::String::FileToString::load(fragment);
		}

		m_engine_api.load_shader(shader_program_name, {
