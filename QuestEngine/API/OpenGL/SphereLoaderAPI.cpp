...
namespace QuestEngine::API::OpenGL {
	...
	void SphereLoader::load_sphere_into_resource(QuestEngine::API::EngineAPI& engine_api, const std::string& sphere_resource_id, const std::string& loaded_shader_id, const bool position_only, const Model::ModelDrawMode draw_mode) const {
		...
		if (position_only) {
			...
			model_loader.load_model(sphere_resource_id, loaded_shader_id, { m_sphere.getVerticesVector() }, { m_sphere.getIndiciesVector() }, { 3 }, draw_mode);
		} else {
			// Load Position, Normals, and Texture Coordinates
			model_loader.load_model(sphere_resource_id, loaded_shader_id, { m_sphere.getInterleavedVerticesVector() }, { m_sphere.getIndiciesVector() }, { 3, 3, 2 }, draw_mode);
		}
	}

	void SphereLoader::load_default_textured_sphere_into_resource(QuestEngine::API::EngineAPI& engine_api, const std::string& sphere_resource_id, const std::string& loaded_shader_id, const Model::ModelDrawMode draw_mode) const {

		// Load into resource
		load_sphere_into_resource(engine_api, sphere_resource_id, loaded_shader_id, false, draw_mode);
