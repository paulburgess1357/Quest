...
namespace QuestEngine::API {
	...
	void RegistryAPI::load_model_into_world(const std::string& entity_id, Model::StandardModel* model, const glm::vec3& world_position, const ECS::RenderPass render_pass) const {
		m_engine_registry_manager.load_model_into_world(entity_id, model, world_position, render_pass);
	}

	void RegistryAPI::load_model_into_world(const std::string& entity_id, Model::IndexedModel* model, const glm::vec3& world_position, const ECS::RenderPass render_pass) const {
		m_engine_registry_manager.load_model_into_world(entity_id, model, world_position, render_pass);
	}
